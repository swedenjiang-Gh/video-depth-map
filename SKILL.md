...
When the requested object or garment is missing from a structural guide, first search the authorized local asset library: for a short-drama project, start with the project's approved `D:\MediaStudio\AIShortDrama\Projects\<project>\` assets, then `D:\MediaStudio\assets\`; then extract a suitable guide. If none exists, offer image generation to create a labeled guide; ask the user only after both routes fail. Do not scan arbitrary disks or invent that a missing guide was present.
